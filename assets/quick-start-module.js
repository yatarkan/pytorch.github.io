...
var opts = {
  ...
  language: 'python',
  ptbuild: 'stable',
  'torch-compile': null, // TODO Consider setting default option
};
...
function getTorchCompileCommand(optionId) {
  const idTobackendMap = {
    inductor: 'inductor',
    aot_cudagraphs: 'cudagraphs',
    onnxrt: 'onnxrt',
    openvino: 'openvino',
    tensorrt: 'tensorrt',
    tvm: 'tvm',
  ...
  return `torch.compile(model, backend="${idTobackendMap[optionId]}")`;
}
...
function addTorchCompileCommandNote(selectedOptionId) {
  ...
  $("#command").append(
    `<pre> # Torch Compile usage: ${getTorchCompileCommand(selectedOptionId)} </pre>`
  );
}
...
function selectedOption(option, selection, category) {
  ...
  if (category === "pm") {
    ...
    // TODO Consider disabling unsupported torch compile options based on other selected options
  }
